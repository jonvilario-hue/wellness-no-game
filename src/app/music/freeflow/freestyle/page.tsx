...
import { Play, Pause, X, RotateCcw, Activity, Mic, Sparkles, Music } from 'lucide-react';
import { cn } from '@/lib/utils';

export default function FreestyleSandboxPage() {
  const [gameState, setGameState] = useState<'idle' | 'recording' | 'review'>('idle');
  const [activeBeat, setActiveBeat] = useState('Boom Bap');
            ...
            {gameState === 'idle' && (
              <div className="w-full space-y-8 text-center">
                <div className="space-y-4">
                  <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">Select Your Loop</p>
                  <div className="flex flex-wrap justify-center gap-2">
                    {['Boom Bap', 'Trap', 'Lo-Fi', 'Dancehall', 'None'].map(beat => (
                      <Button 
                        key={beat} 
                        variant={activeBeat === beat ? 'default' : 'outline'}
              ...
              <div className="w-full space-y-12">
                <div className="flex justify-between items-center">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-sm font-black uppercase tracking-widest">Mic Active: {activeBeat}</span>
                  </div>
            ...
            {gameState === 'review' && (
              <div className="text-center space-y-8 animate-in zoom-in-95">
                <div className="space-y-2">
                  <h3 className="text-2xl font-black uppercase">Flow Captured</h3>
