...
/**
 * Renders the compact 3-column stats bar for consistency metrics.
 */
export function DrawingDashboard() {
  const [mounted, setMounted] = useState(false);
  const { logs, streak, getTopDiscipline } = useDrawingStore();
  ...
  if (!mounted) {
    return <div className="h-24 w-full animate-pulse bg-muted/20 rounded-xl" />;
  }

  return (
    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
      <AssistantTooltip text="Consecutive days of drawing. Motor-perceptual skills degrade without daily hand-eye recalibration.">
...
/**
 * Renders the Skill Wheel and Studio Strategy cards.
 */
export function DrawingAnalytics() {
  const [mounted, setMounted] = useState(false);
  const { getDisciplineBalance, getDaysSinceLastPractice } = useDrawingStore();
