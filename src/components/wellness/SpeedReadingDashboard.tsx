...
import { 
  ...
  Trophy, BookOpen, Brain, Activity, Info
} from 'lucide-react';
...
export function SpeedReadingDashboard() {
  ...
  const aggregateStats = useMemo(() => {
    if (logs.length === 0) return { avgWpm: 0, avgErr: 0, totalWords: 0 };
    ...
    return {
      ...
      totalWords: logs.length * 250 // Rough estimate
    };
  }, [logs]);
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <Card className="bg-primary border-none text-primary-foreground">
          <CardContent className="p-4 flex flex-col items-center justify-center text-center">
            ...
            <p className="text-2xl font-black">{aggregateStats.avgErr}</p>
            <p className="text-[10px] font-bold uppercase tracking-widest opacity-70">True Efficiency (ERR)</p>
          </CardContent>
        </Card>

        <Card className="bg-primary/5 border-primary/10 flex flex-col justify-center p-4 text-center">
          <AssistantTooltip text="Rank based on your average WPM. Move through tiers: Foundational > Proficient > Advanced > Elite.">
            <p className={cn("text-xl font-black", getSpeedRank(aggregateStats.avgWpm).color)}>
              {getSpeedRank(aggregateStats.avgWpm).label}
            </p>
            <p className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Reading Rank</p>
          </AssistantTooltip>
        </Card>
      </div>
