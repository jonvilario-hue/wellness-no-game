...
export function MusicDashboard() {
  const { streak, getWeeklyVolume, getTopDomain, getGlobalHAR, achievements, getTotalCreations } = useMusicStore();
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
        ...
        <AssistantTooltip text="Global Weighted Accuracy: calculated as (Correct / Total) * Difficulty Multiplier. This integrates scores from Listen games, Sing drills, and Theory challenges into a single laboratory index.">
          <Card className="bg-primary/5 border-primary/10 h-full">
            <CardContent className="p-4 flex flex-col items-center justify-center text-center">
      ...
      <div className="space-y-2">
        <h3 className="text-[10px] font-black uppercase tracking-widest text-muted-foreground px-1 flex items-center gap-2">
          <Sparkles className="w-3 h-3" /> Laboratory Achievement Vault
        </h3>
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-3">
          {vaultItems.map(item => (
            <AssistantTooltip key={item.key} text={`Your peak performance in ${item.label}. This accounts for difficulty level and accuracy, mapping your current skill ceiling in this domain.`}>
